...
// duplica string removendo espaços iniciais/finais 
...
char* formatarData(const char* in) {
    ...
    // separo em 3 partes (mês, dia, ano) 
...
char** extrairLista(const char* entradaBruta, int* contador) {
    *contador = 0;
    ...
    // corto espaços nas extremidades 
    char* clean = strtrimdup(temp);
    if (*clean=='\0'){ free(clean); return NULL; }

    // conto quantos elementos numero de vírgulas + 1
    ...
    while (tok){
        lista[*contador]=strtrimdup(tok); // recorto espaços do elemento 
        (*contador)++;
        tok=strtok_r(NULL,",",&ctx);
    }
...
// quebra linha CSV em até 14 campos (considerando aspas) 
    ...
    *contadorCampos = 0;
    ...
    for (size_t k=0; k<=n; k++){
        ...
        if (ch=='"') asp = !asp;
            ...
            // campos especificos que vinham com aspas adicionais 
...
// libera memoria alocada de um game
...
void listaInit(ListaGame* L){ L->primeiro=NULL; L->ultimo=NULL; L->tamanho=0; }

// insere no inicio da lista 
void inserirInicio(ListaGame* L, Game* x){
    No* no = (No*)malloc(sizeof(No));
    no->jogo = x; no->prox = L->primeiro;
    L->primeiro = no;
    if (L->tamanho==0) L->ultimo = no;
    L->tamanho++;
}
...
void inserirFim(ListaGame* L, Game* x){
    No* no = (No*)malloc(sizeof(No));
    no->jogo = x; no->prox = NULL;
    if (L->tamanho==0){ L->primeiro = L->ultimo = no; }
    else { L->ultimo->prox = no; L->ultimo = no; }
    L->tamanho++;
}

// insere em posição especifica
void inserirPos(ListaGame* L, Game* x, int pos){
    if (pos<0 || pos> L->tamanho-1) return; 
    if (pos==0){ inserirInicio(L,x); return; }
    No* ant = L->primeiro;
    for (int i=0;i<pos-1;i++) ant = ant->prox;
    No* no = (No*)malloc(sizeof(No));
    no->jogo = x; no->prox = ant->prox;
    ant->prox = no;
...
Game* removerFim(ListaGame* L){
    if (L->tamanho==0) return NULL;
    if (L->tamanho==1) return removerInicio(L);
    No* ant = L->primeiro;
    while (ant->prox != L->ultimo) ant = ant->prox;
    Game* r = L->ultimo->jogo;
    free(L->ultimo);
    ant->prox = NULL;
    L->ultimo = ant;
    L->tamanho--;
...
Game* removerPos(ListaGame* L, int pos){
    if (L->tamanho==0 || pos<0 || pos>=L->tamanho) return NULL;
...
// lê csv e carrega array de Games na memoria 
...
// busca pelo id no array carregado 
...
int main() {
    ...
    char buf[256];
    // le ids iniciais até linha "FIM"
    while (fgets(buf,sizeof(buf),stdin)) {
        size_t L=strlen(buf); if (L && buf[L-1]=='\n') buf[L-1]='\0';
        if (strcmp(buf,"FIM")==0) break;
        int id = converteParaInt(buf);
        Game* jogoEncontrado = buscarPorId(base,total,id);
        if (jogoEncontrado) inserirFim(&lista, jogoEncontrado);
    }
    ...
    for (int k=0;k<nCmd;k++){
        ...
        if (strncmp(buf,"II",2)==0){
            ...
            Game* jogoEncontrado = buscarPorId(base,total,id);
            if (jogoEncontrado) inserirInicio(&lista, jogoEncontrado);
            ...
            Game* jogoEncontrado = buscarPorId(base,total,id);
            if (jogoEncontrado) inserirPos(&lista, jogoEncontrado, pos);
    ...
    for (No* p=lista.primeiro; p; p=p->prox) imprimirInfoJogo(p->jogo);

    // libera os nós 
    No* p=lista.primeiro;
    while (p){ No* nx=p->prox; free(p); p=nx; }

    // libera todos os jogos carregados 
    for (int i=0;i<total;i++) liberarMemoriaJogo(&base[i]);
    free(base);
    return 0;
}
